...
instancePropertyController.setData = (pageInstance, pageData, POST, REFERRER) => {
  ...
  if (property === 'url' && runtimeInstance.url === '/') {
    pageInstance = produce(pageInstance, draft => {
      draft.redirect = getUrl('admin.instance', {
  ...
  if (schemaProperties[property].enum) {
    const enumMap = schemaProperties[property].enumMap || {}
    items = []
    valueType = 'radios'
...
instancePropertyController.postValidation = (pageInstance, pageData, postRedirect) => {
  ...
  const instanceUrl = pageData.getUserDataProperty('previouspage') || pageInstance.previouspage
