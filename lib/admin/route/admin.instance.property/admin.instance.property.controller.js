...
instancePropertyController.setData = (pageInstance, pageData, POST) => {
  ...
  if (typeof value !== 'string' && property !== 'required') {
...
instancePropertyController.postValidation = (pageInstance, pageData, postRedirect) => {
  ...
  const objectString = typeof value === 'string' && value.match(/^\{[\s\S]*\}$/)
  if (propertyType === 'number') {
    value *= 1
  } else if (propertyType === 'boolean') {
    value = value === 'true'
  } else if (propertyType === 'array' || propertyType === 'object' || value === undefined || objectString) {
    if (value !== undefined) {
