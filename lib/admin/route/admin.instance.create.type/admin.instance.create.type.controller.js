...
instanceCreateTypeController.setData = (pageInstance, pageData, POST) => {
  ...
  const {type, instanceType, addId, addProperty, addPosition} = pageData.getUserData()

  let instanceDisplayType = 'component'

  let types = []
  if (instanceType.match(/^(page|component|field|grouping|control)$/)) {
  ...
  const typedPageInstance = produce(pageInstance, draft => {
    if (type) {
      ...
      console.log('draft.redirect', draft.redirect)
    } else {
      const nameNode = jp.query(draft, '$..[?(@.name === "type")]')[0]
