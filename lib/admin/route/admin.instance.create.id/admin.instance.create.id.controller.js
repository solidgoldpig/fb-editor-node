...
instanceCreateIdController.setData = (pageInstance, pageData) => {
  ...
  if (idSeedValue) {
    ...
    idPrefix = idPrefix.replace(/:id/g, '.').replace(/\//g, '.').replace(/\.(\.)+/g, '.')
  }
...
instanceCreateIdController.validate = (pageInstance, pageData, POST) => {
  ...
  if (idSeed && POST) {
    if (!idSeedValue) {
      ...
      const schema = getServiceSchema(_type)
      const schemaProperty = schema.properties[idSeed]
      // Hack to allow express params in url strings
      // should fix to sort id
      schemaProperty.pattern = schemaProperty.pattern.replace(/w-_/, 'w-_:')
