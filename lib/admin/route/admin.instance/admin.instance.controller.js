...
instanceController.setData = (pageInstance, pageData, POST) => {
  ...
  const contentProps = allInstanceProps.filter(prop => prop.category === 'content')
        ...
        if (_id !== instanceIn._id) {
          ...
          if (pageInstance.infoList) {
            pageInstance = produce(pageInstance, draft => {
              ...
              draft.infoTitle = `${instanceTypeTitle}s require the following ${pageInstance.infoList.length > 1 ? 'properties' : 'property'} to be set`
