...
const {getNavigation, getPagesMethods} = require('@solidgoldpig/fb-runner-node/lib/route/route')
const defaultOrder = [
  '_id',
  '_type',
  '_isa',
...
instanceController.setData = (pageInstance, pageData, POST) => {
  ...
  const mainProps = allInstanceProps.filter(prop => {
    ...
    if (prop.category === 'main') {
      return true
    } else if (prop.category) {
      return false
    }
    if ((uiCategory.main || []).includes(prop.property)) {
      return true
    }
    let isNotMain
    Object.keys(uiCategory).forEach(key => {
      if (key === 'main') {
        return
      }
      if ((uiCategory[key] || []).includes(prop.property)) {
        isNotMain = true
      }
    })
    return !isNotMain
    // if (!)
    // return (uiCategory.main || []).includes(prop.property)
