...
const {getNavigation, getPagesMethods} = require('@ministryofjustice/fb-runner-node/lib/middleware/routes-metadata/routes-metadata')
...
const getDataStructure = (instance, instanceId, pageData, heading) => {
  ...
  Object.keys(schemaProperties).forEach(property => {
    ...
    if (schemaProperty.items && schemaProperty.items._name) { // === 'definition.component') {
      const structureItems = instance[property] ? instance[property].map(propertyInstance => getDataStructure(propertyInstance, instanceId, pageData)) : []
      ...
      structure.structure = [{
        componentTypeTitle: schemaProperty.title,
        componentType: property,
        components: [getDataStructure(instance[property], instanceId, pageData)]
      }]
    }
  })
  if (heading) {
...
const setAdminPanels = (pageInstance, runtimeInstance, discreteInstance, pageData, options = {}) => {
  ...
  const dataStructure = getDataStructure(discreteInstance, _id, pageData, structureHeading)
  ...
  pageInstance = produce(pageInstance, draft => {
    ...
    draft.asideComponents = asideComponents
    return draft
  })
  return pageInstance
}
...
instanceController.setData = (pageInstance, pageData, POST) => {
    ...
    pageInstance = produce(pageInstance, draft => {
      const setNavDirection = (direction) => {
        ...
        if (_id) {
          ...
          draft.preview[direction] = navItem
        }
      }
  ...
  pageInstance = produce(pageInstance, draft => {
    ...
    draft.MODE = 'instance'
    draft.EDITMODE = 'instance'
    const navigation = getNavigation(discreteInstance._id) || {}
    draft = setPageInstanceNavigation(draft, navigation)
    return draft
  })
    ...
    .filter(property => {
      ...
      if (schemaProperties.enableSteps) {
        if (schemaProperties.enableSteps.const) {
          ...
          excluded = excluded.concat(['steps', 'stepsHeading'])
        }
      }
  ...
  //     return {
  //       text: `${getInstanceTitle(isaInstance._id)} (${isaInstance._id})`,
  //       href: pagesMethods.getUrl('admin.instance', {_id: isaInstance._id})
  //     }
  //   })
  //   if (!usedBy.length) {
  //     usedBy = undefined
