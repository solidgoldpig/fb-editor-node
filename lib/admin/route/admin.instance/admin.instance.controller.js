...
instanceController.setData = (pageInstance, pageData, POST) => {
  ...
  const validationProps = Object.keys(validationSchema.properties || []).map(validationProp => {
    ...
    if (instanceValue !== undefined || inheritedInstanceValue !== undefined) {
  ...
  const repeatableProps = getInstanceProperty(_id, 'repeatable') ? allInstanceProps.filter(prop => prop.category === 'repeatable') : undefined
