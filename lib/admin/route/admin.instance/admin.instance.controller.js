...
instanceController.setData = (pageInstance, pageData, POST) => {
  ...
  // }
  // const getAllowableTypes = (schemaName, property) => {
  //   let itemsName = []
  //   const schema = getServiceSchema(schemaName)
  //   if (!schema) {}
  //   if (!schema.properties) {}
  //   if (!schema.properties[property]) {}
  //   if (!schema.properties[property].items) {}
  //   const schemaItems = schema.properties[property].items
  ...
  //       allowables.push(...getTypeByCategory(itemName.replace(/^definition\./, '')))
  //     } else {
  ...
  //   return allowables
  // }
  // console.log(getSchemaNameByCategory('page'))
  ...
  const allInstanceProps = Object.keys(schema.properties)
    ...
    .map(property => {
      ...
      if (!schema.required || !schema.required.includes(property)) {
        nestedPropertyTitle += optionalText
      }
      ...
      if (Array.isArray(value)) {
        if (property === 'steps') {
          value = value.map(_id => {
            return `[${_id}](${getUrl('admin.instance', {
              _id
            })})`
          })
        } else {
      ...
      return {
        ...
        value,
        valueSet,
        url: getUrl('admin.instance.property', {
          _id,
          property
        })
      }
