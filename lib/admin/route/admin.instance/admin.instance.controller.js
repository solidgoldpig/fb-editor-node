...
instanceController.setData = (pageInstance, pageData, POST) => {
  ...
  if (runtimeInstance._type.startsWith('page.')) {
    ...
    if (pathToInstance) {
      ...
      usedNodes = usedNodes.reverse().map(_id => {
        ...
        return `[${getInstanceTitle(_id)}](${nodeUrl})`
      })
  ...
  const optionalText = ' <em>(optional)</em>' // ' *(optional)*'
  ...
  const getPropValue = (instanceValue, runtimeInstanceValue, defaultValue) => {
    ...
    if (!value) {
      ...
      // if (defaultValue) {
      // value += ' <i>Default is <b>${defaultValue}</b></i>'
      // }
    }
    return value
  }
  ...
  const allInstanceProps = Object.keys(schema.properties)
    .filter(property => !property.match(/^(_id|_type|validation|errors)$/))
    ...
    .map(property => {
      ...
      if (value === notSet && nestedPropertySchema.default) {
        ...
        value += ` - <strong>defaults to ${nestedPropertySchema.default}</strong>`
      }
  ...
  const getPropBundle = (bundleProp, bundleSchema, bundleValues, bundleInheritedValues, bundleParentProp, bundleNestedProp) => {
    ...
    const runtimeInstanceValue = get(bundleInheritedValues, propertyLookup)
    let defaultValue
    let value = getPropValue(instanceValue, runtimeInstanceValue, defaultValue)
  ...
  const validationProps = Object.keys(validationSchema.properties || []).map(validationProp => {
    ...
    if (errorComponents.length) {
      ...
      validationPropBundle.summary = 'Error strings'
  ...
  const mainProps = allInstanceProps.filter(prop => !prop.category)
...
instanceController.validate = (pageInstance, pageData, POST, ajv) => {
  ...
  pageInstance = setErrors(pageInstance, instanceErrors)
  // pageInstance = produce(pageInstance, draft => {
  //   const header = jp.query(draft, '$..[?(@._id === "admin.instance--header")]')[0]
  //   const goshComp = {
  //     _id: 'fwap',
  //     _type: 'gosh'
  //   }
  //   for (let i = 0; i < 10; i++) {
  //     header.components.push(goshComp)
  //   }
  // })
