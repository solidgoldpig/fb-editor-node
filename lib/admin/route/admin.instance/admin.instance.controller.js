...
const setAdminPanels = (pageInstance, runtimeInstance, discreteInstance, pageData, options = {}) => {
  ...
  pageInstance = produce(pageInstance, draft => {
    ...
    if (buildUrl) {
      draft.adminBack = `${buildUrl === '/' ? '' : buildUrl}/edit`
    }
    ...
    if (_id === 'service') {
      ...
      if (options.property) {
        ...
        if (!draft.adminBack) {
          draft.adminBack = pageData.pagesMethods.getUrl('admin.instance', {_id: 'service'})
        }
      } else {
        delete draft.adminBack
        delete draft.sectionHeading
        heading = 'heading'
      }
      draft[heading] = 'Configure form'
      return draft
    }
